(function (root, factory) {
  ...
  function breakBy(value, separator = /\s+/) {
    ...
    return [value.substr(0, match.index), value.substr(match.index + match[0].length)];
  }
  ...
  Suggester.prototype = {
    ...
    getContext: function (expression) {
      ...
      do {
        ...
        switch (
          context.scope // prepresents previous scope
        ) {
          case null:
            ...
            if (currentToken.length || !rest) {
              ...
              context.scope = this.isFunction(currentToken) ? 'function' : 'field';
            }
            break;
          case 'field':
      ...
      var execFn = function (resolve, reject) {
        ...
        switch (context.scope) {
          case 'field': {
            ...
            Object.keys(this.schema[context.model]['fields']).forEach((field) => {
              ...
              if (fieldSchema['type'] === 'boolean') {
                suggestions.push(new Suggestion('is ' + field + suffix, 'is ' + field));
                suggestions.push(new Suggestion('is not ' + field + suffix, 'is not ' + field));
              }
            });
    ...
    fetchRelated: async function (field, searchTerm) {
      var execFn = function (resolve, reject) {
        ...
        const url =
          field.search_url + '?' + field.search_param + '=' + encodeURIComponent(searchTerm);
